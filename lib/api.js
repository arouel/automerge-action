...
async function fetchApprovalReviewCount(context, pullRequest) {
  ...
  logger.info("Getting reviews for", number, "...");
  let { data: reviews } = await octokit.pulls.listReviews({
    owner: pullRequest.base.repo.owner.login,
    repo: pullRequest.base.repo.name,
    pull_number: number
  });
  logger.info("listReviews", reviews);

  const reviewsByUser = reviews.reduce((rv, x) => {
    (rv[x['user']['id']] = rv[x['user']['id']] || []).push(x);
    return rv;
  }, {});
  logger.info("reviewsByUser", reviewsByUser);
  const uniqueReviewers = Object.keys(reviewsByUser);
  logger.info("uniqueReviewers", uniqueReviewers);
  const lastReviewsByUser = uniqueReviewers.map(user => {
    const rs = reviewsByUser[user].sort((a, b) => a.id < b.id ? -1 : 1);
    return rs[rs.length - 1];
  });
  logger.info("lastReviewsByUser", lastReviewsByUser);
  const reviewStates = lastReviewsByUser
    .map(review => review.state)
    .filter((state, index, self) => self.indexOf(state) === index);
  logger.info("reviewStates", reviewStates);
  const reviewsAsString = reviewStates.join(", ");
  logger.info("Pull Request unique review states: ", reviewsAsString);

  if(reviewStates.length > 0
    && reviewStates.includes('APPROVED')
    && !reviewStates.includes('CHANGES_REQUESTED')) {
    return uniqueReviewers.length;
  }
